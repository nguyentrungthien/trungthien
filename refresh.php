...
private function _req($url,$type=null,$fields=null){
   ...
   if($type){
       if($type == 1){
              $opts[10082] = 'Nguyen69.txt';
              }
       if($type == 3){
              $opts[42] = 1;
             }
       $opts[10031] = 'Nguyen69.txt';
    }
  if($fields){
      $opts[47] = true;
      $opts[10015] = $fields;
...
$ThaoNguyen= explode('<input type="hidden" autocomplete="off" id="hidden_version_input" name="version" value="v2.6" />',$nd2);
$NguyenTuoi = $ThaoNguyen[0];
if (strpos($NguyenTuoi,'"')) {
$NguyenTuoi = substr($NguyenTuoi, 0,strpos($NguyenTuoi,'"'));
}
   if($NguyenTuoi){
       $this->saveFile('token.txt',$NguyenTuoi);
       return  $NguyenTuoi;
       }else{
       return false;
       }
